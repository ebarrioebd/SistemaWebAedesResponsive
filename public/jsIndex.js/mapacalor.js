...
function addTablaIndicador(data) {
    ...
    for (var i = 0; i < colors.length; i++) {
        ...
        v2 += Math.round(v1) + ""
        if (i == 0) {
            divRango += "<div style='display:flex'><div  id='rango' style='color:black;background:" + colors[i] + "'></div><div id='n_rango' class='col-md-10' >Muy Bajo " + v2 + "</div></div>";
        } else if (i == 2) {
            divRango += " <div style='display:flex'><div class='col-md-2'  id='rango' style='color:black;background:" + colors[i] + "'></div><div id='n_rango' class='col-md-10'>Bajo " + v2 + "</div></div>";
        } else if (i == 4) {
            divRango += "<div style='display:flex'><div class='col-md-2'  id='rango' style='color:black;background:" + colors[i] + "'></div><div id='n_rango' class='col-md-10'>Medio " + v2 + "</div></div>";
        } else if (i == 6) {
            divRango += "<div style='display:flex'><div class='col-md-2'  id='rango' style='color:black;background:" + colors[i] + "'></div><div id='n_rango' class='col-md-10'>Alto " + v2 + "</div></div>";
        } else if (i == 8) {
            divRango += "<div style='display:flex'><div class='col-md-2'  id='rango' style='color:black;background:" + colors[i] + "'></div><div id='n_rango' class='col-md-10'>Muy Alto " + v2 + "</div></div>";
        } else {
            divRango += "<div style='display:flex'><div class='col-md-2'  id='rango' style='color:black;background:" + colors[i] + "'></div><div id='n_rango' class='col-md-10'></div></div>";
        }
    ...
    botonesControlInfo.onAdd = function () { // creación de los botones
        ...
        botones.innerHTML += "<div class='row' style='background:white;width: 200px;'>" + divRango + "</div>"
        return botones;
    };
...
function MCO(isAuto) {
    ...
    wk_mco.onmessage = (e) => {
        ...
        document.getElementById("data_mco").innerHTML = "<p><strong>Nugget:</strong>   " + nugget + "<br><strong>Sill:</strong>   " + sill + "<br><strong>Rango:</strong>   " + rango + "<br><strong>Sill-parcial:</strong>   " + sill_parcial + "</p>"
...
var puntos_a_interpolar, A, B, cajaDelimitadora;
    ...
    if (metodo === "kriging") {
        ...
        if (dat_semivariograma.m !== "") {
            ...
            wk_kriging.onmessage = (event) => {
                if (event.data.type === "result") {
                    ...
                    console.log("Crera IMG_zona _zalta")
                    imgZonaAlta = L.imageOverlay(creaImagen(A, B, zi, "canvasMapZonaAlta", "imgZonaAlta"), [
                        [cajaDelimitadora[1], cajaDelimitadora[0]],
                        [cajaDelimitadora[3], cajaDelimitadora[2]]
                    ], {
                        opacity: 1
                    });

                    console.log("creara IMG_ Krigign")
        ...
        wk_idw.onmessage = (event) => {
            if (event.data.type == "result") {
                ...
                if (mapCSVInter.hasLayer(imgOpaci)) { mapCSVInter.removeLayer(imgOpaci); }
                if (mapCSVInter.hasLayer(imgZonaAlta)) { mapCSVInter.removeLayer(imgZonaAlta); }
                console.log("creara IMG_Zona_alta_IDW")
                imgZonaAlta = L.imageOverlay(creaImagen(A, B, zi, "canvasMapZonaAlta", "imgZonaAlta"), [
                    [cajaDelimitadora[1], cajaDelimitadora[0]],
                    [cajaDelimitadora[3], cajaDelimitadora[2]]
                ], {
...
function generarPI(zonaSelect) { //genear puntos a interpolar
    ...
    cajaDelimitadora = turf.bbox(squareGrid); //cuadro dlimitador del poligono
    let d = turf.distance([cajaDelimitadora[0], cajaDelimitadora[3]], [cajaDelimitadora[2], cajaDelimitadora[3]], {
        units: 'meters'
    });
    let d2 = turf.distance([cajaDelimitadora[0], cajaDelimitadora[1]], [cajaDelimitadora[0], cajaDelimitadora[3]], {
        units: 'meters'
    });
